...
reposition = ->
    ...
    force.attr("stroke-width", (d) -> if dist(f for d, f of d.force) > 0 then 10/scale*showforce.checked else 0)
...
repositionfast = ->
    ...
    grad.attr("x1", (d) -> d.x).attr("x2", (d) -> d.x - 50/scale*d.grad.x*nodes.length/s.lp.obj)
        ...
        .attr("stroke-width", (d) -> if 50/scale*dist(l for d, l of d.grad)*nodes.length/s.lp.obj > 0.05 then 10/scale*showgrad.checked else 0)
repositionzoom = ->
    link.attr("stroke-dasharray", (d) -> if d.F then null else 10/scale+","+10/scale)
        .attr("stroke-width",  (d) -> 0.035*d.F or 5/scale*showzero.checked)
