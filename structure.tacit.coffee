...
gen_classes = (nodeLookup, nodeIDLookup, nodeList, beamList, nodes, beams) ->
    class Node
        constructor: (pos) ->
            ...
            # add the node to lookup and list
            @id = nodes++
            nodeLookup[@x] ?= {}
            nodeLookup[@x][@y] ?= {}
            if nodeLookup[@x][@y][@z]? then throw "a node is already there, aborting."
            nodeLookup[@x][@y][@z] = @id
        ...
        constraints: ->
            ...
            for d, fix of @fixed
                ...
                if fix then null
                    ...
                    A: (b.l[d]/b.L for b in @sourced).concat(-b.l[d]/b.L for b in @targeted)
    ...
    class Beam
        constructor: (points...) ->
            # turn points into node_idxs
            [@source, @target] = (nodeIDLookup[i] for i in (getNodeIDX(pt) for pt in points))
            # determine physical characteristics
            @update()
            @diameter = null
            @force = 0
            @grad = {x: 0, y: 0, z: 0}
            # add this to start and end nodes
            @id = beams++
    ...
    LPstring = ->
        ...
        for node in nodeList
            for dim, c of node.constraints() when c isnt null
                lp += "\n  n#{node.id}#{dim}:"
                lp += " #{strsign(a)} #{abs(a)} f#{c.i[j]}" for a, j in c.A when a isnt 0
                lp += " = #{c.b}"
...
class Structure
    # TODO: import / export list of coordinates, fixes etc., list of connections
    constructor: () ->
    ...
    solveSizing: ->
        ...
        for beam in @beamList
            ...
            for dim in ["x", "y", "z"]
                ...
                duals = (@lp["n#{beam.source.id}#{dim}"] or 0) - (@lp["n#{beam.target.id}#{dim}"] or 0)
                beam.grad[dim] = rho*geo*duals
        for node in @nodeList
            for dim in ["x", "y", "z"]
                node.grad[dim] = sum(beam.grad[dim] for beam in node.sourced) - sum(beam.grad[dim] for beam in node.targeted)
...
## TEST 6 # test 3D structures and independent problems
s2 = new Structure
